...
const OrderHistory = () => {
  ...
  return (
    <Box position={"relative"} minHeight={"100vh"}>
      ...
      <VStack spacing={4} align="start" maxWidth={720} marginBlock={"3em"} marginInline={"auto"}>
                ...
                {order.orderHamburgers.map((hamburger: any, index: number) => (
                  <Box key={hamburger.id + index}>
                    <Text fontWeight="bold">
                      {hamburger.name} - ${(Math.round(hamburger.price * 100) / 100).toFixed(2)}
                    </Text>
                    {hamburger.extras.length > 0 && (
                      <VStack spacing={2} align="start" gap={0}>
                        {hamburger.extras.map((extraIngredient: any, index: number) => (
                          <Text key={extraIngredient.id + index}>
                            + {extraIngredient.ingredient.name} ({extraIngredient.count})
                          </Text>
                        ))}
                      </VStack>
                    )}
